...
<div class="modal-shiftfix">
    ...
    <div class="container-fluid main-content">
                ...
                <div class="widget-container1 parent">
                    ...
                    <div class="col-md-2">
                        <select class="form-control CascadeVersion ChangeChart ChangeChart1 model ChangeChart2"
                                name="brand" id="product"
                                data-url="#">
                            <option value="0">所有品牌</option>
                            <c:forEach varStatus="status" items="${listModel}" var="items">
                                <option value="${items.clientId}">${items.clientId}</option>
                            </c:forEach>
                        </select>
                    </div>
                    <div class="col-md_10">
        ...
        <script type="text/javascript">
            $(function () {
                ...
                var model_all = function () {
                    var url = "${basePath}model/model_statistics_ajax";
                    ...
                    $.getJSON(url, {
                        ...
                        if (table != null) {
                            for (var i = 0; i < table.length; i++) {
                                // var url = "http://fota.redstone.net.cn/Home/ModelStatistics/model_details?product_id=" + product_id + "&channel_id=" + channel_id + "&brand=" + table[i].brand + "&model=" + table[i].model + ""
                                // var url1 = "http://fota.redstone.net.cn/Home/VersionStatistics/version_area?product_id=" + product_id + "&channel_id=" + channel_id + "&brand=" + table[i].brand + "&model=" + table[i].model + "&statistics_type1=1"
                                // var url2 = "http://fota.redstone.net.cn/Home/VersionStatistics/version_area?product_id=" + product_id + "&channel_id=" + channel_id + "&brand=" + table[i].brand + "&model=" + table[i].model + "&statistics_type1=2"
                                // var url3 = "http://fota.redstone.net.cn/Home/VersionStatistics/version_area?product_id=" + product_id + "&channel_id=" + channel_id + "&brand=" + table[i].brand + "&model=" + table[i].model + "&statistics_type1=3"
                                var url = "${basePath}model/model_details?brand=" + table[i].brand + "&model=" + table[i].model;

                                var url1 = "${basePath}device/version_area?brand=" + table[i].brand + "&model=" + table[i].model + "&version=" + table[i].version + "&statistics_type1=1"
                                var url2 = "${basePath}device/version_area?brand=" + table[i].brand + "&model=" + table[i].model + "&version=" + table[i].version + "&statistics_type1=2"
                                var url3 = "${basePath}device/version_area?brand=" + table[i].brand + "&model=" + table[i].model + "&version=" + table[i].version + "&statistics_type1=3"

                                html += '<tr><td>' + table[i].brand + '</td><td><a href="' + url + '">' + table[i].model + '</a></td><td><a href="' + url2 + '">' + table[i].new + '</a></td><td><a href="' + url1 + '">' + table[i].count + '</a></td><td>' + table[i].percentage + '</td><td><a href="' + url3 + '">' + table[i].total + '</a></td></tr>'
                            }
                            ;
                        }
                ...
                var model_ajax = function () {
                    var url = "${basePath}model/model_statistics_ajax"
                    // var url = "http://fota.redstone.net.cn/Home/ModelStatistics/model_statistics_ajax?product_id=764&channel_id=664";
                    var brand = $('select[name="brand"]').val();
                    var select = $('.parent').find('select');
                    var title = '';
                    for (var i = select.length - 1; i >= 0; i--) {
        ...
        <script>
            $(function () {
                var new_title = "<div class='font_lit'><span class='green_hover'>机型统计：</span>" + Message.model_m + "</div><div class='font_lit'><span class='green_hover'>日活量：</span>" + Message.hover3 + "</div><div class='font_lit'><span class='green_hover'>" + Message.hover7 + "：</span>" + Message.hover1 + "</div><div class='font_lit'><span class='green_hover'>" + Message.hover6 + "：</span>" + Message.hover4 + "</div><div class='font_lit'><span class='green_hover'>" + Message.time_f + "：</span>" + Message.time_f_data + "</div><div class='font_lit'><span class='green_hover'>" + Message.time_type + "：</span>" + Message.time_type_data + "</div><div class='font_lit'><span class='green_hover'>说明：</span>" + Message.model_des_data + "</div>";

                var new_title1 = "<div class='font_lit'><span class='green_hover'>机型概括：</span>以列表的形式展现各品牌的机型概括情况</div><div class='font_lit'><span class='green_hover'>机型：</span>" + Message.model_data + "</div><div class='font_lit'><span class='green_hover'>昨日活跃量：</span>" + Message.y_new + "</div><div class='font_lit'><span class='green_hover'>昨日新增量：</span>" + Message.y_new + "</div><div class='font_lit'><span class='green_hover'>新增活跃占比（%）：</span>" + Message.new_active + "</div><div class='font_lit'><span class='green_hover'>" + Message.hover7 + "：</span>" + Message.hover1 + Message.y_new + "</div>";
