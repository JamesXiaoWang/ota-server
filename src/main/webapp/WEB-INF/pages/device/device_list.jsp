    ...
    <div class="col-lg-12">
        <div class="widget-container fluid-height clearfix">
            ...
            <div class="grid-structure">
                ...
                <div class="row">
                    <form method="get" action="${basePath}device/queryDeviceNumberInfo" autocomplete="on"
                          id="uploadForm"
                        ...
                        <div class="col-md-9">
                            <div class="form-group">
                                <div class="col-xs-6 col-sm-4">
                                    ...
                                    <label class="input-group-btn">
                                        <a href="${basePath}device/device_list_add">
                ...
                <div style="text-align: center">
                    <ul class="pagination">
                        <li
                            ...
                            <a href="<c:url value="${basePath}device/device_list?pageNo=1"/> ">«</a>
                        </li>
                        <li>
                            <!--如果当前页数是第一页，那么点上一页仍然是当前页。 -->
                            <a href="<c:url value="${basePath}device/device_list?pageNo=${pageInfo.pageNum==1?pageInfo.pageNum:pageInfo.pageNum-1}"/> ">上一页</a>
                        </li>
                        ...
                        <c:forEach begin="1" end="${pageInfo.pages >= 10 ? 10 : pageInfo.pages}" step="1" var="pageNo">
                            <li
                                ...
                                <a href="<c:url value="${basePath}device/device_list?pageNo=${pageNo}"/> ">${pageNo}</a>
                            </li>
                        </c:forEach>
                        <li>
                            <!--如果当前页数是最后一页，那么点击下一页仍然是当前页。 -->
                            <a href="<c:url value="${basePath}device/device_list?pageNo=${pageInfo.pageNum==pageInfo.pages?pageInfo.pageNum:pageInfo.pageNum+1}"/> ">下一页</a>
                        </li>
                        <li
                                <c:if test="${pageInfo.pageNum == pageInfo.pages}">
                                    class="disabled"
                                </c:if>>
                            <a href="<c:url value="${basePath}device/device_list?pageNo=${pageInfo.pages}"/> ">»</a>
                        </li>
                    </ul>
                </div>
...
<script>
    ...
    function update(id, imeinumber, index) {
        console.log("index：" + index + "\t" + "imei:" + imeinumber)
        window.location.href = "${basePath}device/device_list_edit?imei=" + imeinumber + "&id=" + id + "&pageNo=" + index;
    }
    ...
    $("#deleteHaulBtn").click(function (event) {
        ...
        $.ajax({
            url: '/ota-server/device/deleteDeviceNumber',
            dataType: 'json',
            type: 'GET',
            async: false,
            data: {id},
            success: function (data) {
                ...
                if ("SUCCESS" == data.status) {
                    ...
                    v.click(function () {
                        window.location.href = "${basePath}device/device_list?pageNo=${pageInfo.pageNum}";
                    })
                }
            },
            error: function (response) {
